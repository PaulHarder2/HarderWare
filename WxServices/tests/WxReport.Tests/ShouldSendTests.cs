...
// Exercises the priority order (first -> gap -> scheduled -> arrival pre-filter)
// and the two behaviours the review and acceptance criteria care about most:
// the post-gap arrival send (finding #1) and the once-per-slot scheduled cadence.
...
public class ShouldSendTests
{
    ...
    private static RecipientConfig Rcpt(string? hours, string tz = "UTC")
        => new() { Timezone = tz, ScheduledSendHours = hours };
    ...
    {
        var state = new RecipientState { RecipientId = "r" };
        var (send, reason, severity, allowSkip) = ReportWorker.ShouldSend(
            Rcpt(""), state, new InputIdentity("m", "none", "none"), Cfg(), new DateTime(2026, 5, 31, 9, 0, 0, DateTimeKind.Utc));
    ...
    public void PostGap_AdvancedInput_SendsOnceGapClears()
    {
        ...
        var inGap = ReportWorker.ShouldSend(Rcpt(""), state, advanced, Cfg(minGap: 60), sentAt.AddMinutes(10));
        Assert.False(inGap.send);
        Assert.Equal("gap", inGap.reason);

        // After the gap clears: the advance still reads as changed, so it sends.
        var afterGap = ReportWorker.ShouldSend(Rcpt(""), state, advanced, Cfg(minGap: 60), sentAt.AddMinutes(61));
    ...
    public void NoInputChange_PreFilterSkips()
    {
        ...
        var (send, reason, _, _) = ReportWorker.ShouldSend(
            Rcpt(""), state, identity, Cfg(minGap: 60), sentAt.AddMinutes(120));
    ...
    public void ScheduledHourPassed_NotYetServed_Sends()
    {
        ...
        var (send, reason, _, allowSkip) = ReportWorker.ShouldSend(
            Rcpt("7"), state, new InputIdentity("m", "none", "none"), Cfg(), new DateTime(2026, 5, 31, 7, 30, 0, DateTimeKind.Utc));
    ...
    public void ScheduledSlotAlreadyServed_DoesNotResendInSameSlot()
    {
        ...
        var (send, reason, _, _) = ReportWorker.ShouldSend(
            Rcpt("7"), state, identity, Cfg(), new DateTime(2026, 5, 31, 9, 0, 0, DateTimeKind.Utc));
    ...
    public void ScheduledOnlySend_ThenArrivalWithinGap_StillRateLimited()
    {
        ...
        var (send, reason, _, _) = ReportWorker.ShouldSend(
            Rcpt(""), state, new InputIdentity("m_new", "none", "none"), Cfg(minGap: 60), sentAt.AddMinutes(5));
