...
3. **`InstallRoot`** — an in-memory value (`AddInstallRoot`, `WxServices.Common`) projecting the env-aware runtime root so `IConfiguration["InstallRoot"]` is authoritative even inside a container (WX-64/65/66).
