...
- **Vocabulary** — Per-language editor over the `LanguageTemplates` table (WX-233). Pick a language (English is excluded — it is the source, not an editable target) and edit each token's `Phrase` and `Note`; Token, English source, Context, and Representable are read-only. Validation is **live** (the "Mirion pattern", via `INotifyDataErrorInfo`): a phrase must be non-empty and must preserve the English source's `{n}` placeholders (`TemplateValidation.PlaceholdersMatch`), and **Save stays disabled until every edited cell is valid**.
