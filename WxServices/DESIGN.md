...
Three services share a log directory and a SQL Server database. WxParser.Svc feeds the database; WxReport.Svc reads from it; WxMonitor.Svc watches both. (WxParser runs as a native Windows service; the other three headless services run as Linux containers — see *Containerized deployment (WX-7)*.)
