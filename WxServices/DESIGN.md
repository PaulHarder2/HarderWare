...
Every build input for the four service images is pinned so a rebuild months later produces the same runtime, closing the drift CodeRabbit flagged on the WX-65 PR (a rebuild could otherwise pull a newer .NET patch, ODBC driver, Python runtime, or rendering library even though `requirements.txt`'s top-level versions are fixed). This extends the WX-17 observability-image pinning to the **service** images. Three layers:
...
- **apt packages → exact versions.** `wxparser` pins `libgfortran5`/`libgomp1`; `wxvis` pins its full set (`curl`, `gnupg`, `ca-certificates`, `python3`, `python3-pip`, `fonts-dejavu`, `fonts-liberation`, `unixodbc`, and `msodbcsql17` from the Microsoft repo). The rolling `packages-microsoft-prod.deb` is checksum-verified (`sha256sum -c`) before it registers the MS repo. `wxmonitor`/`wxreport` install no apt packages, so they carry base-digest pins only.
...
**The deliberate-bump procedure.** The pins are exact, so they are meant to be updated on purpose, not to drift. A rebuild will **fail loudly** when Debian supersedes a pinned apt version and drops the old one from the mirror — the accepted trade of exact-version pins over a Debian snapshot repo: the failure is visible, never a silently-different image. To bump:
...
Validating the pinning itself is a **clean rebuild of each image** from the pinned inputs (`docker compose build`); there is no separate functional test (Test Proc = N/A).
