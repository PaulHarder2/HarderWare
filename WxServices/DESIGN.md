...
Secrets (`Claude:ApiKey`, `Smtp:Username`, `Smtp:Password`, `Smtp:FromAddress`) are read from the `GlobalSettings` database row (Id = 1), with `C:\HarderWare\appsettings.local.json` and a local `appsettings.local.json` beside the executable as fallbacks. `What3Words:ApiKey` remains the file-based exception, configured directly in `appsettings.local.json` — an inconsistency worth retiring, since it is the one secret still expected to live in a file.
