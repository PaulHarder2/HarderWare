...
- **Deterministic backstop.** After `submit_reconciled_report` on an unscheduled cycle, `ReportWorker` suppresses the send when the reconciled snapshot is materially identical to the last sent one (`ForecastSnapshotBody.MateriallyEquals` — categorical fields exact, temperature within tolerance, wind within tolerance **or in the same impact band** so trivial same-band drift is suppressed, WX-110), or when the only difference is a severe-flag **de-escalation** on an observation-only advance with no newer GFS run or TAF (`MateriallyEqualsIgnoringSevere` + `HasSevereEscalationOver` — severe-flag hysteresis). The hysteresis is one-directional: a severe hazard *appearing* (`severeFlag` false→true) is always news and is never suppressed, even on a bare observation — only an unsupported *removal* is held back. It also suppresses the send when the reconciled report carries no computed change (`StructuredReportBody.Changes` empty, WX-506): the significance gate reads the provisional forecast and can pass a change that reconciliation drops, and an update with no computed change has no "Why this update" band and nothing new to say. This rule can withhold an update whose only severe change the detector does not count — a severe block the prior never covered, or one beyond the detector's 120-hour change horizon — so the hysteresis's "never suppressed" above does not extend to it; the scheduled report still shows such a block. The one exception is a severe hazard appearing on a baseline reset (the prior dropped after a schema bump), which is sent without a band rather than withheld. Suppressions persist the reasoning trace, leave the committed anchor unchanged, and are counted in telemetry (`wxreport.suppressed.redundant.total`, `wxreport.suppressed.severe_flip.total`, `wxreport.suppressed.no_computed_change.total`). This is idempotence/stability, not significance judgment — Claude still owns the "is it news?" decision.
...
**Per-block local labels (WX-504):** the user message also carries a `block_local_labels` section, built by `BlockLocalLabels` from the provisional snapshot. It lists every block's `startUtc` with its local weekday and date, day-part (early hours / morning / afternoon / evening) and local clock span. The prompt's day-part and window-labelling rules take a block's day and day-part from this section, and tell the model never to convert a `startUtc` to local time itself. The WX-340 production watch showed that conversion failing in shipped prose (a 00:00–06:00 block called "morning", a window shifted a whole day) while the prompt rules against both were already in force. Labels come from the local wall clock, never from `startUtc + 6h`, because blocks are aligned to local day-part boundaries (WX-155) and a DST day's 00:00–06:00 block spans 5 or 7 UTC hours. The day-part names are English and canonical, like `day_name_reference`'s date anchors; the localized words still come from `day_name_reference` and the approved-vocabulary glossary. Deterministic, per report, no LLM call.
...
| `SignificanceGate` | WxReport.Svc | WX-114 deterministic **cost pre-filter**: compares the current provisional snapshot against the last *sent* forecast and reports whether anything changed materially (per local day, through `DayFigures`: high/low magnitude, directional freeze/thaw at 32 °F, heat crossing, peak sustained-wind advisory + magnitude; per 6-hour block: precip occurrence/type, severe flag — across four horizon tiers). `ReportWorker` skips the Claude call when nothing fires. TAF-aware (WX-160): the evaluated "current" is a GFS+TAF merge (`TafBlockProjector`), so a fresh TAF that moves nothing material is suppressed deterministically instead of bypassing the gate via the retired `taf-fresh` shortcut. Wind is sustained-only; a gust gates only through the 50-kt severe rule. Suppress-only and mode-controlled (`Report:SignificanceGate:Mode` — Off / Shadow / Enforce, default **Enforce**); never decides sends |
...
- **Configure** — Settings editor that writes **only to the database** (WX-315). Operational settings go to the `Config` table, which every service reads through the WX-313 provider; secrets go to the `GlobalSettings` row. Both are committed in a **single transaction**, so a refused write persists neither. The tab writes **no configuration file at all** — the file-based path that used to reach only the native apps is gone, which is what closes the WX-307 single-source-of-truth gap and retires the WX-69 stopgap header.

  **Editable:** SMTP host and port, alert email (→ `Config`); SMTP username/password/from-address and the Claude API key (→ `GlobalSettings`). A malformed SMTP port **fails the save** rather than silently substituting 587, and a blank field writes **no row** — so an unset value keeps falling through to the shared default instead of becoming an explicit override.

  **Read-only** (shown, never written): Install Root (resolved from `appsettings.shared.json` / `WXSERVICES_INSTALL_ROOT`); the foundational location values — home ICAO, lat/lon, bounding box, the four region bounds — and the map extent, all owned by the setup console (WX-314) and changed by re-running it; `Claude:Model`, which resolves from `appsettings.shared.json` (making it DB-editable is WX-317); and the **native / management connection string**, which is bootstrap-critical — it is how the database is reached at all, so the app that needs it to start is not where it can be broken. Read-only fields remain **selectable and copyable**: a value longer than its box must still be readable and copyable.

  Panels: Paths, Home Location and Fetch Region *(set by the setup script)*, Database *(native / management connection)*, Email/SMTP (+ Test button that sends a real email), Claude API (+ Test button), Map Rendering *(set by the setup script)*, Monitoring. After a successful save the tab calls `IConfigurationRoot.Reload()` and re-reads its fields, so what is displayed is what is in effect rather than what was typed; it then switches to the Setup tab and re-runs prerequisite checks.
...
**Configuration:** Follows the same layered pattern as the services, with the DB `Config` table layered **last** so it wins (WX-313). `appsettings.shared.json` supplies cross-project defaults (`Smtp`, `Claude`, `Fetch`, `Report:DefaultLanguage`, `Report:DefaultScheduledSendHour`); WxManager-specific non-secret settings (`WxManager:` section — station lookup radius, display limits, default timezone, AWC endpoint, User-Agent, success-banner timing) live in WxManager's own `appsettings.json`; anything present in `Config` overrides both.

**The secrets rule (WX-322).** *Secrets live **only** in the database — the `GlobalSettings` row (Id = 1). The single exception is the **connection string**, which cannot live in the store it unlocks and therefore stays in the per-environment `appsettings.local.json`.*

That exception is the same bootstrap logic `BootstrapKeys` applies to non-secret configuration, and it is the whole of it: one secret, for one reason. `ClaudeApiKey`, `GeminiApiKey`, `SmtpUsername`, `SmtpPassword`, `SmtpFromAddress` and `What3WordsApiKey` are all columns on `GlobalSettings`.

The rule is not merely tidiness. `What3Words:ApiKey` was file-based until WX-322, and the Configure tab's pre-WX-315 save path — which rebuilt `appsettings.local.json` from a fixed key list — **silently deleted it**, leaving `///` address lookups broken from roughly 2026-06-08 until 2026-07-20 with nothing to indicate why. A file that some other component rewrites is not a safe home for a credential.

Secrets are read **at point of use** from `GlobalSettings` (as `AnnouncementTab` does for SMTP and `RecipientsTab` does for What3Words), not captured into `App` statics at startup — so a value changed on the Configure tab takes effect without a restart.

**Bootstrap-critical keys stay file-sourced** and are never carried in `Config`: `ConnectionStrings:` (circular — the provider needs it to reach the very table it would read the override from), `Database:StartupRetry:` (governs reaching the DB, consumed before the overlay loads), `Telemetry:` (wired at host-build time), and exactly `Claude:TimeoutSeconds`. The rule has one definition, `MetarParser.Data.Configuration.BootstrapKeys`, enforced on the read side by the provider and on the write side by `ConfigStore` — so the Configure tab and the setup console cannot write a key the provider would ignore, which would otherwise produce a row that looks configured and has no effect.
...
| `appsettings.local.json` | **No** | Per-machine overrides. Since WX-315 the Configure tab does **not** write it — it is authored by the setup console (WX-314) and holds the connection string plus any per-machine values not yet in the `Config` table. **The setup console merges this file, it does not rebuild it** (WX-326): a re-run sets `ConnectionStrings:WeatherData` and leaves every other key untouched, matching the operator's own key casing, and refuses to write at all if the file cannot be read or parsed. This file has no committed `.example` — it is the operator's, so setup may not assume it knows the whole of it. **The four container files are still rebuilt from their `.example` templates, and that is a known open defect, not a design choice: any operator-set value they hold is blanked on a re-run (`Monitor.GrafanaServiceAccountToken` is a live example). Tracked as WX-357 — do not read this row as saying the container files are safe to regenerate.** |
...
**Secrets** (`GlobalSettings` table, Id = 1): managed via WxManager → Configure tab. Stored in the database, not in configuration files. Fields: `ClaudeApiKey`, `GeminiApiKey` (translation-QA judge — WX-235), `SmtpUsername`, `SmtpPassword`, `SmtpFromAddress`, `What3WordsApiKey` (WX-322). Services read these at the start of every cycle. There is no file-based secret exception; the connection string is the only credential still in `appsettings.local.json`, for the reason given in §4.7.
...
**Noticing and responding to transitive apt drift.** The transitive gap is *silent* — a drifted transitive dependency does **not** fail the build — so it is caught by comparison, not by error. Two things bound and expose it: the base-image digest already freezes the base OS, so the **only** moving surface is the transitive closure of the apt-installed packages (`wxvis`'s render stack + `msodbcsql17`; `wxparser`'s two `wgrib2` libs; nothing in `wxmonitor`/`wxreport`); and that surface is inspectable — capture the installed-package manifest from a known-good image (`docker run --rm wxvis:dev dpkg -l > manifest.txt`) and diff it after a rebuild to see exactly which packages moved. For `wxvis`, the **sample-render check against the host reference** (bump step 3) is the functional canary: a render regression after an otherwise-unchanged rebuild points at drift. *If drift causes a problem,* pin the offending transitive package by adding it to that Dockerfile's apt install list with `=version` (the same mechanism as the direct pins, extended to the culprit); if drift recurs or resists a quick pin, that is the trigger to adopt the full snapshot/closure approach (the deferred Option 2 — its own ticket, not this one).
