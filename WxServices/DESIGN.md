...
6. When all 121 hours are stored, mark the run `IsComplete = true` and purge old runs (retaining the most recent `Gfs:RetainModelRuns`, deployed as 1). Completeness is decided by **set membership** over `0..MaxForecastHours`, never by a count of distinct hours — WX-451; a count is satisfied one-for-one by any hour stored outside the expected range, and by hours belonging to a different run. The purge never deletes a run newer than the latest complete one, which at a retention of 1 is what keeps a complete run available while the next is still downloading.
