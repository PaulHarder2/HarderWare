...
| `SignificanceGate` | WxReport.Svc | WX-114 deterministic **cost pre-filter**: compares the current provisional snapshot against the last *sent* forecast and reports whether anything changed materially (per local day: high/low magnitude, directional freeze/thaw at 32 °F, heat crossing; per 6-hour block: precip occurrence/type, severe flag, sustained-wind advisory + magnitude — across four horizon tiers). `ReportWorker` skips the Claude call when nothing fires. Suppress-only and mode-controlled (`Report:SignificanceGate:Mode` — Off / Shadow / Enforce, default **Enforce**); never decides sends |
