...
2. Set the ticket's **Test Proc** to the procedure path at this point; **Test Result** records the PASS later, after the §13 in-production run (it is legitimately empty until then — a structural pre-deploy run is a `WAIT`, never a PASS).
3. Exempt changes (pure-docs / pure-config / pure-process / pure-tooling — nothing functional to verify) skip this, same as the §7b test-run exemption.
