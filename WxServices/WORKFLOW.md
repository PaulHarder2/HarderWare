...
The **general working principles** that govern how we operate across *every* project — our ticketing, grooming, review, verification, and release discipline (the operating agreement between the maintainer and the AI collaborator) — are deliberately **not** restated here. They live in a shared standing policy outside this repository (`$AI_SHARED_ROOT/WORKFLOW-CORE.md`, read at the start of every working session); that policy is internal working context, **not part of this repository and not subject to code review**. A contributor needs the mechanics below; the shared principles are context, not a repo artifact. (The discriminator that draws this line — a rule is in-repo iff a code review must be able to see it — and the retired embed-and-re-sync model it replaced are recorded in `$AI_SHARED_ROOT/CONVENTIONS.md` → *Route before you write*; see WX-282.)

> **`$AI_SHARED_ROOT`** is the maintainer's shared-context tree, which lives outside this repository and is not distributed with it. **An external contributor cannot read those files and does not need to** — everything a contributor or reviewer must know is in this document. The variable is named rather than a filesystem path because that tree sits in a different place on each machine. *(These two documents previously lived under `~/.claude/shared/`; they moved in WX-372, and this reference was repointed in WX-407.)*
...
4. **Running it — read one step at a time.** *Added 2026-07-04 (Paul).* Don't hand Paul the whole doc to flip through against the app. Drive the run **interactively**: read one numbered step aloud, Paul performs it and reports the result; on a failure, note it and **continue** (don't stop). At the end, present the **list of failed steps** — that list is the run's deliverable, and it decides the Test Result (a PASS only when the list is empty) plus any fix/follow-up. The `WX-NN.md` doc stays the authored, versioned script you read from.
...
`dotnet format WxServices.CI.slnf --verify-no-changes` must also exit clean before push. CI runs this on every PR and a format-only failure costs a full CI round-trip plus a CodeRabbit-equivalent cycle on a mistake that is trivial to fix locally. *Added 2026-05-19 after WX-72's first CI run failed on two trailing-newline drifts.* **This is a distinct step — a green build is not a proxy for it.** `EnforceCodeStyleInBuild` runs the code-style *analyzers* on every build, but `dotnet format --verify-no-changes` also enforces `WHITESPACE` *layout* rules (object-initializer wrapping, indentation) that a clean `dotnet build`/`dotnet test` does **not** catch — so "build green, tests green" can still fail the CI format gate (exit 2). Run the verify explicitly; if it flags, `dotnet format` (no `--verify`) auto-fixes in place, then confirm `git status` shows only intended files. *Reinforced 2026-07-06 after WX-253's first CI run failed on new-test object-initializer whitespace despite a green local build + 923 passing tests.*
...
Step 6 above (auto-apply at startup) is fine for fast migrations, but a migration that **rewrites a large table** — a size-of-data change such as widening a column's type, adding a `NOT NULL` column with a default across millions of rows, or re-clustering — is still best applied out-of-band — though **not** because the services would collide. `EnsureSchemaCoreAsync` serialises migration work across all four with an exclusive `sp_getapplock` (`WxServices.MigrateAsync`, session-scoped), so concurrent DDL cannot happen. The real hazard is that lock's **120-second timeout**: a size-of-data rewrite runs far longer than two minutes, so while the first service holds the lock the other three fail to acquire it, throw, exit, and are restarted by `restart: unless-stopped` — a restart loop for the whole duration, on top of the system being unavailable throughout.
